...
var hashing = require('n-multi-hashing');
...
var hash = function (headerHashBuf, nonceBuf, blockHeight) {
  var mixOutBuf = Buffer.alloc(32, 0);
  var hashOutBuf = Buffer.alloc(32, 0);
  hashing.kawpow(headerHashBuf, nonceBuf, blockHeight, mixOutBuf, hashOutBuf);
  var mix = mixOutBuf.toString('hex');
  var output = hashOutBuf.toString('hex');
  return {
      output: output,
      mix: mix,
      buffers: {
...
var JobManager = module.exports = function JobManager(options) {
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    const { output, mix } = hash(header_hash, miner_given_nonce, job.rpcData.height);
    
    if(mix == miner_given_mixhash) result='true';
